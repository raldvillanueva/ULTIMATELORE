...
export default function NewWorkOrders() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] flex-col gap-4">
      ...
      {editForm && <><div className="fixed inset-0 z-40 bg-black/20" onClick={closeEdit} /><aside className="fixed right-0 top-0 z-50 flex h-full w-[500px] flex-col bg-white shadow-2xl"><header className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-4"><div><p className="text-xs font-medium uppercase tracking-wide text-slate-400">New Work Order</p><h2 className="font-mono text-lg font-bold text-slate-800">{editForm.field_order_no || `ID #${editForm.id}`}</h2></div><div className="flex items-center gap-2"><button onClick={updateWorkOrder} disabled={saving} className="flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white hover:bg-blue-700 disabled:opacity-60"><Save size={14} />{saving ? 'Saving...' : 'Update'}</button><button onClick={closeEdit} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X size={18} /></button></div></header><div className="flex-1 space-y-6 overflow-y-auto px-5 py-5">{SECTIONS.map(([title, fields]) => <section key={title}><h3 className="mb-3 border-b border-slate-100 pb-2 text-xs font-bold uppercase tracking-widest text-slate-400">{title}</h3><div className="grid grid-cols-2 gap-3">{fields.map(([key, label, type]) => <label key={key} className={key === 'location' || key === 'remarks' ? 'col-span-2' : ''}><span className="mb-1 block text-xs font-medium uppercase text-slate-500">{label}</span>{key === 'remarks' ? <textarea rows={3} value={editForm[key] ?? ''} onChange={event => setField(key, event.target.value)} className={`${inputClass} resize-none`} /> : SELECT_OPTIONS[key] ? <select value={editForm[key] ?? ''} onChange={event => setField(key, event.target.value)} className={inputClass}><option value="">— Select —</option>{SELECT_OPTIONS[key].map(option => <option key={option}>{option}</option>)}</select> : <input type={type || 'text'} value={editForm[key] ?? ''} onChange={event => setField(key, event.target.value)} className={inputClass} />}</label>)}</div></section>)}</div></aside></>}
    </div>
  )
}
